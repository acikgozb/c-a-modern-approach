// Modify the `inventory.c` program in Section 16.3 so that the `p` (print)
// command calls `qsort` to sort the `inventory` array before it prints the
// parts.
...
void print(void);
// SOLUTION (2):
// Declare the comparison function so that it can be reference anywhere
// from this file.
...
void print(void) {
  ...
  // SOLUTION (1):
  // Execute `qsort` before printing.
  qsort(inventory, num_parts, sizeof(struct part), cmp);
...
// The function should compare the parts by their numbers.
int cmp(const void *p1, const void *p2) {
