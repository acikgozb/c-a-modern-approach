...
//
// `queue_type` is an incomplete structure type.
// Create a file named `queueADT.c` that contains the full definition of
// `queue_type` as well as definitions for all the functions in `queue.h`.
// Use a fixed-length array to store the items in a queue. Create a file named
// `queueclient.c` that creates two queues and performs operations on them. Be
// sure to provide `create` and `destroy` functions for your ADT.
